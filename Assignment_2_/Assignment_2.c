...
static void* Thread(void *inArgs)
{
	...
	for(int i = 0; i < 3; i++){
		...
		*/
		...
		results[tid].thread_deadline 		= next.tv_nsec/MICRO_SECOND_MULTIPLIER;
		results[tid].thread_response_time 	= (results[tid].thread_end_time.tv_nsec - results[tid].thread_start_time.tv_nsec)/MICRO_SECOND_MULTIPLIER;
		
		trace_write("RTS_Thread_%d Terminated The absolute response time: %lld", 
		results[tid].thread_number, 
		results[tid].thread_response_time, 
		results[tid].thread_deadline);
...
int main(int argc, char **argv)
{
	...
	int priorities[NUM_THREADS]= {4, 2, 3, 1}; /* Priorities are in the range from 1 to 99 when it comes to SCHED_FIFO and SCHED_RR, and for  
